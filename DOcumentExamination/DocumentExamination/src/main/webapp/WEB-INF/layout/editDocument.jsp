    ...
    <sf:form method="POST" commandName="documentForm" cssClass="contact_form" enctype="multipart/form-data">
        ...
        <ul>
            ...
            <li>
                <!--????? ????????? type, ????? ??????? ????????? ???????????? ????, ????????????? ??? Html5
                           cssStyle - ????? Css-->
                <sf:label path="startDate">Дата подписания: </sf:label> <sf:input cssClass="on_top" path="startDate" type="text" onfocus="this.select();lcs(this)"  onclick="event.cancelBubble=true;this.select();lcs(this)" class="calendar3"/>  <sf:errors path="startDate" />
                </li>
                <li>
